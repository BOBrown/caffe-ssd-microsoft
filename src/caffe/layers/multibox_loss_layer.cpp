...
void MultiBoxLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  loc_bottom_vec_.push_back(&loc_pred_); // loc_bottom_vec_
...
void MultiBoxLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  GetGroundTruth(gt_data, num_gt_, background_label_id_, use_difficult_gt_,
                 &all_gt_bboxes); //将gt存放在all_gt_bboxes变量中，此处将batch size的全部gt全部赋值给了all_gt_bboxes

  // Retrieve all prior bboxes. It is same within a batch since we assume all
  // images in a batch are of same dimension.
  vector<NormalizedBBox> prior_bboxes;
  vector<vector<float> > prior_variances;
  GetPriorBBoxes(prior_data, num_priors_, &prior_bboxes, &prior_variances);//将prior_data赋值给了prior_bboxes，将剩余的值赋值给prior_variances
  ...
  num_matches_ = 0;
  int num_negs = 0;
  //以下的for循环是依次遍历num_,即batch size，来求与GT匹配的prior box。注意本次是求prior box与GT的matches
  for (int i = 0; i < num_; ++i) {
    ...
    if (!use_prior_for_matching_) {
      ...
      // Use prior bboxes to match against all ground truth. 我们采用的是使用prior box 来与GT匹配
      ...
      if (share_location_) {//我们采用share_location_
    ...
    all_match_indices_.push_back(match_indices); //这里将所有与gt匹配的prior_box保存到all_match_indices_ ， 使用map变量是因为，<int,vector<>>中的int代表了label，只不过在shared location中label使用为-1
    all_neg_indices_.push_back(neg_indices);  //这里将没有与gt匹配的prior_box保存到all_neg_indices_
  }  //for循环结束
  ...
  if (num_conf_ >= 1) {
    ...
    conf_loss_layer_->Forward(conf_bottom_vec_, conf_top_vec_); //经过此次forward之后，类成员变量conf_loss_就有了值
  }
