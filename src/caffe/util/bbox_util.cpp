...
void MatchBBox(const vector<NormalizedBBox>& gt_bboxes,
  ...
  match_indices->clear();
  match_indices->resize(num_pred, -1); //将match_indices中的值全设置为-1
  ...
  if (label == -1) {
    ...
    for (int i = 0; i < num_gt; ++i) {
      gt_indices.push_back(i); //这里保存GT的索引号，用vector<int>的形式保存
    }
  } else {
  ...
  if (num_gt == 0) {  //没有gt，则返回这个函数
    return;
  }

  //有gt，并且gt_indices保存了gt的索引号，开始计算匹配的prior boxes

  //num_pred代表prior box的个数，第0个prior box与第0,1,2...个gt判断IOU
  ...
  for (int i = 0; i < num_pred; ++i) {
    for (int j = 0; j < num_gt; ++j) {
      ...
      if (overlap > 1e-6) {
        ...
        overlaps[i][j] = overlap; //overlap[i][j]表示第i个prior box与第j个gt的iou值
      }
    }
  }
  ...
  vector<int> gt_pool;
  for (int i = 0; i < num_gt; ++i) {
    gt_pool.push_back(i);//gt_pool存储所有gt的id
  }
  while (gt_pool.size() > 0) {
    ...
    if (max_idx == -1) {
      ...
      (*match_indices)[max_idx] = gt_indices[max_gt_idx]; //若 match_indices[124] = 5 ，代表第124个prior box和第5个gt匹配
...
void GetGroundTruth(const Dtype* gt_data, const int num_gt,
  ...
  for (int i = 0; i < num_gt; ++i) {
    ...
    (*all_gt_bboxes)[item_id].push_back(bbox); //对MAP插入数据，由于是map->second是vector，因此使用push_back插入
  }
}
